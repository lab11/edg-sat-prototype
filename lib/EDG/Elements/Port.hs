...
embedPort' mapLens transformCons n pd@PortDesc{..} = do
  ...
  errContext (context uid) $ do
    ...
    constrain $ (Val pconstrained) :=>
        ((Not (Val pconnected :: Exp EDG)) :==
...
-- | to be called at the end of the construction phase on every port.
constrainModPortConn :: Ref ModPort -> EDGMonad ()
constrainModPortConn = constrainPortConn modulePortInfo "modulePortInfo"

-- | to be called at the end of the construction phase on every port.
constrainLinkPortConn :: Ref LinkPort -> EDGMonad ()
constrainLinkPortConn = constrainPortConn linkPortInfo "linkPortInfo"

-- | to be called at the end of the construction phase on every port.
constrainPortConn :: Lens' GS (Map (Ref a) (PortInfo a))
                  -> String
                  -> Ref a -> EDGMonad ()
constrainPortConn lens s p = errContext context $ do
  -- retieve the portInfo data
  pi@PortInfo{..} <- getPortInfo' s lens p
  let options = map (\ c -> (Val piPConnectedTo :== Val c :: Exp EDG))
                  piPConnections
  constrain $ (Val piPConstrained :: Exp EDG)
      :=> (Val piPConnected :=> (Any options))
  where
    context = "constrainPortConn `" ++ s ++ "` `" ++ show p ++ "`"
