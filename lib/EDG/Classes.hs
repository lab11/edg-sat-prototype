...
class ( BoundedJoinSemiLattice t
      , PartialOrd t
      ) => Constraint t where
  ...
  validate :: t -> Constraining t -> Bool
...
--           have to define.
collapseEq :: (Constraint t, Eq t) => t -> Constraining t -> Bool
